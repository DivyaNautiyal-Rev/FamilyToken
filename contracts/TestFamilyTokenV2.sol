...
// File: FamilyToken/IPancakeRouter01.sol
...
// File: FamilyToken/IPancakeRouter02.sol
...
// File: FamilyToken/IPancakePair.sol
...
// File: FamilyToken/IPancakeFactory.sol
...
contract Ownable is Context {
    ...
    constructor() internal {
        ...
        _owner = 0xE7d54E479Db9E0e0b978a943af1E01f097340234;
        // _owner = 0x1FC040b5FE107847ad8cA4939FF92c3d5f03cF25;
        //  _owner = 0x892607bEF2b3c5d70917ee57FD1Ae55f38A63873;
...
// File: FamilyToken/BUSDManager.sol
...
contract TestFamilyTokenV2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;
    ...
    address private constant _busd = 0xbf4279a9D0A2C584337127a4911DCe99b1fdc893;
    ...
    address public charityWallet = 0xd99933E3001Dd3663680441da0bF0205a735639a;
    address public marketingWallet = 0x7475F23D5D7e666171E6311f3C09540Ec6978eeD;
    address public arbitrageWallet = 0xa9EA970A3DDB27ad3Fb791573C0a4b0F82e93256;
    ...
    uint256 public minTokensBeforeSwap = 10 * 10**18;
    ...
    constructor() public {
        _rOwned[0xE7d54E479Db9E0e0b978a943af1E01f097340234] = _rTotal;
        BUSD = IERC20(_busd);
        ...
        address pancakeV2PairBNB = IPancakeFactory(pancakeV2Router.factory())
            .createPair(address(this), pancakeV2Router.WETH());
        isBlacklisted[pancakeV2PairBNB] = true;
        ...
        emit Transfer(
            address(0),
            0xE7d54E479Db9E0e0b978a943af1E01f097340234,
            _tTotal
        );
    }
    ...
    function _swapAndLiquify(uint256 liquidityBalance)
        private
        lockTheSwap
        returns (bool feeTransferred)
    {
        _lock = true;
        feeTransferred = false;
        uint256 half = liquidityBalance.div(2);

        uint256 otherHalf = liquidityBalance.sub(half);
        uint256 tokensForSwapAmount = _accountFee.arbitrageAmount.add(half);
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 busdAmount) private {
        ...
        BUSD.approve(address(pancakeV2Router), busdAmount);
    ...
    function _transferFeesToWallets(
        ...
        _transfer(address(this), charityWallet, _accountFee.charityAmount);
        _transfer(address(this), marketingWallet, _accountFee.marketingAmount);

        uint256 arbitrageAmount = busdAmount
            .mul(_accountFee.arbitrageAmount)
    ...
    function _buyToken(uint256 amount) private {
        _accountFee.totalAmount = _accountFee.totalAmount.add(
            amount.mul(_getBuyFee()).div(10**4)
        );
        _accountFee.liquidityAmount = _accountFee.liquidityAmount.add(
            amount.mul(buyFeeLiquidity).div(10**4)
        );
        _accountFee.charityAmount = _accountFee.charityAmount.add(
            amount.mul(buyFeeCharity).div(10**4)
        );
    ...
    function _sellToken(uint256 amount) private {
        if (_sender != owner() && !_lock && _takeFee) {
            ...
            _accountFee.totalAmount = _accountFee.totalAmount.add(
                amount.mul(_getSellFee()).div(10**4)
            );

            _accountFee.liquidityAmount = _accountFee.liquidityAmount.add(
                amount
                    .mul(_isPenalty ? penaltyFeeLiquidity : sellFeeLiquidity)
                    .div(10**4)
            );

            _accountFee.charityAmount = _accountFee.charityAmount.add(
                amount.mul(_isPenalty ? penaltyFeeCharity : sellFeeCharity).div(
                    10**4
                )
            );
            _accountFee.marketingAmount = _accountFee.marketingAmount.add(
                amount
                    .mul(_isPenalty ? penaltyFeeMarketing : sellFeeMarketing)
                    .div(10**4)
            );
            _accountFee.arbitrageAmount = _accountFee.arbitrageAmount.add(
                amount
                    .mul(_isPenalty ? penaltyFeeArbitrage : sellFeeArbitrage)
                    .div(10**4)
            );
        }
        ...
        return
            _isPenalty
                ? penaltyFeeLiquidity
                    ...
                    .add(penaltyFeeArbitrage)
                : sellFeeLiquidity
                    .add(sellFeeCharity)
                    .add(sellFeeMarketing)
                    .add(sellFeeArbitrage);
    }
    ...
    function setBlacklistAddress(address account, bool value)
        external
        onlyOwner
    {
    ...
    function setCharityWallet(address newWallet) external onlyOwner {
        require(
            newWallet != address(0),
            'Charity wallet can not equal zero address'
        );
    ...
    function setArbitrageWallet(address newWallet) external onlyOwner {
        require(
            newWallet != address(0),
            'Arbitrage wallet can not equal zero address'
        );
